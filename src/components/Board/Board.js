...
class Board extends React.Component {
  ...
  onPanResponderMove = (event, gesture) => {
    try {
      const { draggedItem, movingMode, pan, startingX } = this.state;
      ...
      if (movingMode) {
        ...
        if (startingX + gesture.dx < -20 && gesture.vx < 0) {
          this.carousel.snapToPrev();
        }
        if (
          startingX + gesture.dx + CARD_WIDTH - 20 > deviceWidth &&
          gesture.vx > 0
        ) {
    ...
    this.movingSubscription = setTimeout(() => {
      ...
      if (columnId - 1 === columnIndex) {
        ...
        this.setState({
          ...
          startingX: x,
          startingY:
            dy - boardPositionY - STATUSBAR_HEIGHT - 80 - (ios ? 0 : dy - y),
        });
        this.rotate(MAX_DEG);
      }
    }, 200);
  ...
  movingStyle = (zIndex) => {
    ...
    const interpolatedScaleAnimation = rotate.interpolate({
      inputRange: [-MAX_RANGE, 0, MAX_RANGE],
    ...
    return {
      ...
      transform: [
        { translateX: pan.x },
        { translateY: pan.y },
        { scale: interpolatedScaleAnimation },
  ...
  renderWrapperRow = (data) => (
    <Card {...data} {...this.props} width={CARD_WIDTH} />
  );
